...
/* 
         let opciones = ["Label", "Description"];  */
      /*   var opciones = ["Label", "Description"]; */

      const opciones = capturarSeleccionados();
   ...
   /*   let label ="Label",
     description = "Description"; */
       ...
       coincidenciasPorItem = opciones.filter(opcion =>{
          ...
          const coincide = palabrasBusqueda.some(palabra => opcionLower.includes(palabra));
          
          if (coincide) {
            // Generar una porción relevante de texto para cada término de búsqueda
            item.Highlighted = palabrasBusqueda
    ...
    if(coincidenciasPorItem.length >0){
      return true;
         ...
         /*  return coincide; */ // Mostrar solo si todas las palabras coinciden 
         
          /* })
              const labelLower = quitarTildes(item.Label.toLowerCase());
              // Verificar que todas las palabras coincidan en Description
              const coincideLabel = palabrasBusqueda.some(palabra => labelLower.includes(palabra));
              
              if (coincideLabel) {
                // Generar una porción relevante de texto para cada término de búsqueda
                item.HighlightedLabel = palabrasBusqueda
                .map(palabra => extractContext(labelLower, palabra, item.Label))
                .join("...");
  /* 
                  // Generar una porción relevante de texto para cada término de búsqueda
                  item.HighlightedDescription = palabrasBusqueda
   ...
   */
            ...
            return coincideLabel;*/ // Mostrar solo si todas las palabras coinciden
    ...
    contenedorResultados.classList.add("bg-display");
        ...
        resultadoFiltrado.forEach(item => {
           ...
           /* h3.innerHTML = `${item.Label}`; */
             h3.innerHTML = resaltarTexto(item.HighlightedLabel || item.Label, inputBuscador.value);
...
function extractFields(data) {
  ...
  function displayFields(fields) {
    ...
    fieldsContainer.innerHTML = "<h2>Selecciona los campos:</h2>";
